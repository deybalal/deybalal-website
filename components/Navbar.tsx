...
import { Home, Disc, Mic2, ListMusic, Heart } from 'lucide-react';
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 h-screen w-20 hidden md:flex flex-col items-center py-8 glass z-50 transition-all duration-300 hover:w-64 group overflow-hidden">
      ...
      <div className="flex flex-col gap-6 w-full">
        {navItems.map((item) => (
          <Link
            key={item.name}
            href={item.href}
            className="flex items-center px-6 py-3 text-gray-400 hover:text-white hover:bg-white/10 transition-colors duration-200 relative"
          >
